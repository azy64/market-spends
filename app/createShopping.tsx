...
const createShoppingScreen = () => {
    ...
    const [lastSelectedItem, setLastSelectedItem] = useState<{ id: string, ref: any }>({ id: "", ref: null });
    ...
    const deleteItem = (id: string) => {
        if (currentShoppingListId) {
            ...
            const newOne = {
                ...currentShoppingListId, items: newItems,
    ...
    }, [currentShoppingListId, numberOfItems,shoppingLists]);
    return (
        <View>
            ...
            <View>
                ...
                <ScrollView style={{ height: Dimensions.get("window").height * 75 / 100, backgroundColor: "#f7f3f3ff" }}>
                    {currentShoppingListId.items &&
                        currentShoppingListId.items.map((item: any, index: number) => (
                                ...
                                <ItemTemplate
                                    key={index}
                                    item={item}
                                    onSwipeableOpen={(e:any) => swipeableDirection(item.id, e)}
